...
export const StudyGroup = () => {
  ...
  const [joinGroupError, setJoinGroupError] = useState<{message: string, groupName: string, groupId: string} | null>(null);
  ...
  const getMemberAvatars = (members: GroupMember[]) => {
    return members.slice(0, 3).map((member, index) => (
      <Avatar key={member.userId} className="w-8 h-8 -ml-2 first:ml-0 border-2 border-background">
    ...
    <div className="space-y-6">
      ...
      {showGroupDetails && selectedGroup && (
        ...
        >
          <motion.div
            ...
            className="glass-card rounded-xl p-6 w-full max-w-4xl max-h-[90vh] overflow-hidden"
                  ...
                  {(groupMessages[selectedGroup.id] || []).map((message) => (
                    <div key={message.id} className={`flex ${message.userId === 'current-user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] p-3 rounded-lg ${
                        message.type === 'system' 
                          ? 'bg-blue-500/10 text-blue-600 text-center text-sm'
              ...
              <TabsContent value="members" className="space-y-4">
                <div className="grid gap-4">
                  {selectedGroup.members.map((member) => (
                    <div key={member.userId} className={`flex items-center justify-between p-4 rounded-lg ${
                      member.isCreator ? 'bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/20' : 'bg-muted/20'
                    }`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleChatAccess(group)}
                      >
      ...
      {joinGroupError && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"
          onClick={() => setJoinGroupError(null)}
        >
          <motion.div
            initial={{ scale: 0.9, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
          ...
          >
            ...
            <div className="flex gap-3">
              <Button 
                onClick={() => handleJoinGroup(joinGroupError.groupId)}
                className="flex-1"
              >
                Join Now
              </Button>
